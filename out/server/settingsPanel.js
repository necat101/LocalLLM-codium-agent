...
class SettingsPanel {
    ...
    static createOrShow(extensionUri, serverManager) {
        ...
        const panel = vscode.window.createWebviewPanel('agenticSettings', 'Agentic LLM Settings', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async browseForModel() {
        ...
        if (result && result[0]) {
            ...
            // Save to settings
            await vscode.workspace.getConfiguration('agentic').update('llamaCpp.modelPath', modelPath, vscode.ConfigurationTarget.Global);
            // Load the model
    ...
    async saveSetting(key, value) {
        await vscode.workspace.getConfiguration('agentic').update(key, value, vscode.ConfigurationTarget.Global);
    ...
    getHtmlContent(config, status) {
        ...
        const repeatPenalty = config.get('sampling.repeatPenalty', 1.1);
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
