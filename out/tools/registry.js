...
class ToolRegistry {
    ...
    registerBuiltinTools() {
        ...
        this.register({
            ...
            execute: async (args, context) => {
                ...
                const isAllowed = context.allowedCommands.some(allowed => baseCommand === allowed || baseCommand.endsWith(`\\${allowed}.exe`) || baseCommand.endsWith(`/${allowed}`));
        ...
        this.register({
            ...
            execute: async (args, context) => {
                const filePath = this.resolvePath(args.path, context.workspaceRoot);
        ...
        this.register({
            ...
            execute: async (args, context) => {
                const dirPath = this.resolvePath(args.path, context.workspaceRoot);
        ...
        this.register({
            ...
            execute: async (args, context) => {
                ...
                const directory = this.resolvePath(args.directory, context.workspaceRoot);
    ...
    // Helper Methods
    // ─────────────────────────────────────────────────────────────
    resolvePath(inputPath, workspaceRoot) {
        if (path.isAbsolute(inputPath)) {
            return inputPath;
        }
        return path.join(workspaceRoot, inputPath);
    }
    async listDir(dirPath, maxDepth, currentDepth) {
    ...
    async searchInFiles(directory, pattern, filePattern) {
        const results = [];
