...
let llmClient: LlamaCppClient | undefined;
let serverManager: ServerManager | undefined;
let statusBarItem: vscode.StatusBarItem | undefined;
let modelStatusBarItem: vscode.StatusBarItem | undefined;
let chatStatusBarItem: vscode.StatusBarItem | undefined;
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const endpoint = config.get<string>('llamaCpp.endpoint', 'http://localhost:8080');
    const autoStart = config.get<boolean>('llamaCpp.autoStart', true);
    ...
    llmClient = new LlamaCppClient({
        endpoint,
        maxTokens: config.get<number>('model.maxTokens', 4096),
        temperature: config.get<number>('sampling.temperature', 0.7),
        topP: config.get<number>('sampling.topP', 0.9),
        topK: config.get<number>('sampling.topK', 40),
        frequencyPenalty: config.get<number>('sampling.repeatPenalty', 1.1)
    });
    ...
    chatProvider = new ChatViewProvider(context.extensionUri, llmClient, tools);
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(ChatViewProvider.viewType, chatProvider)
    );

    // Initialize agent
    agentChat = new AgentChat(llmClient, tools);
    ...
    try {
        ...
        if (!isServerRunning && autoStart) {
            ...
            const lastModel = context.globalState.get<string>('lastModelPath');
            if (lastModel) {
                ...
                setTimeout(async () => {
                    const action = await vscode.window.showInformationMessage(
                        'Agentic: No LLM model configured. Select a model to start.',
                        'Select Model',
                        'Open Settings',
                        'Later'
                    );
        ...
        setTimeout(() => {
            vscode.window.showWarningMessage(
                'Agentic: Could not connect to LLM server.',
                'Open Settings'
            ).then(action => {
...
function registerCommands(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('agentic.openSettings', () => {
            if (serverManager) {
                SettingsPanel.createOrShow(context.extensionUri, serverManager);
            }
        }),
...
function updateStatusBar(isConnected: boolean, modelName?: string): void {
    if (!statusBarItem || !modelStatusBarItem) return;
