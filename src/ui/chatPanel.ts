                ...
                try {
                    ...
                    if (!hasWriteFile) {
                        ...
                        if (codeBlock && codeBlock[1].trim().length > 50) {
                            ...
                            const extMap: Record<string, string> = { rust: 'rs', rs: 'rs', c: 'c', cpp: 'cpp', python: 'py', py: 'py', javascript: 'js', js: 'js', typescript: 'ts', ts: 'ts', html: 'html', css: 'css', java: 'java', go: 'go', php: 'php', ruby: 'rb', swift: 'swift', kotlin: 'kt', sh: 'sh', bash: 'sh', json: 'json' };
                            ...
                            if (promptLower.includes('atkin')) fileName = `sieve_of_atkin.${ext}`;
                            else if (promptLower.includes('eratosthenes')) fileName = `sieve_eratosthenes.${ext}`;
                            else if (promptLower.includes('fibonacci')) fileName = `fibonacci.${ext}`;
                            else if (promptLower.includes('hello')) fileName = `hello.${ext}`;
                            else if (promptLower.includes('sort')) fileName = `sort.${ext}`;
                    ...
                    for (const tc of allToolCalls) {
                        ...
                        let args: any = {};
                        try {
                            args = JSON.parse(this.sanitizeJson(tc.function.arguments as string));
                        } catch (e) {
                            ...
                            if (dummyCalls.length > 0) {
                                try {
                                    args = JSON.parse(this.sanitizeJson(dummyCalls[0].function.arguments as string));
                                } catch { /* skip */ }
                            }
                        }
    ...
    private rescueToolCall(jsonStr: string, calls: ToolCall[]): void {
        ...
        if (toolName === 'write_file' || toolName === 'append_file') {
            ...
            if (pathMatch) {
                ...
                if (jsonStr.toLowerCase().includes('atkin')) path = 'sieve_of_atkin.rs';
                else if (jsonStr.toLowerCase().includes('eratosthenes')) path = 'sieve_eratosthenes.rs';
            }
